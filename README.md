...
Your score is **how many of the 14 tracked assets are nailed** — no formulas, no weights. Two of the medals have one extra requirement.

| Medal | Name | Count | Also required | What it means |
|-------|------|-------|---------------|---------------|
| 🥉 | **Getting Started** | 1–4 of 14 | — | A few of the files exist. Nothing in the repo tells an agent how it works |
| 🥈 | **On Track** | 5–8 of 14 | — | Real scaffolding is in place, but the conventions are still in people's heads |
| 🥇 | **Solid** | 9–11 of 14 | `AGENTS.md` nailed | The conventions are written down, in the one file every tool reads |
| 🏆 | **AI-Ready** | 12–14 of 14 | every 🤖 AI Context asset nailed | Conventions, boundaries, reviewers and procedures all live in the repo |
...
| **`.github/agents/*.agent.md`** | Three **adversarial reviewers** you assign to a PR — one checks the diff against what the issue actually asked for, one checks whether the tests would have failed against the old code, and one checks what a revert would not undo. Each asks a question that can come back _no_, works from the diff alone, and has no way to say _ship it_ |
